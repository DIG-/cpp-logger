...
void test_level(const Level level) {
  ...
  Util::Union<2> logger_string(std::make_unique<ForwardingString>(level, tag, message, exception, source, count),
                               std::make_unique<ForwardingString>(level, tag, message, exception, source, count));
  ...
  count = 0;
  Util::Union<2> logger_callable(
      std::make_unique<ForwardingCallable>(level, tag, message_of_function, exception, source, count),
      std::make_unique<ForwardingCallable>(level, tag, message_of_function, exception, source, count));
